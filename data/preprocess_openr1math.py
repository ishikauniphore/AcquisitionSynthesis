...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--local_save_dir", default="/home/ubuntu/AcquisitionSynthesis/data/openr1math/", help="The save directory for the preprocessed dataset."
    )
    ...
    full_dataset = datasets.load_dataset("open-r1/OpenR1-Math-220k", "all")['train']
    full_dataset = full_dataset.rename_column("problem", "query")
    full_dataset = full_dataset.rename_column("solution", "reasoning")
    full_dataset = full_dataset.rename_column("answer", "response")
    ...
    def create_dataset(ds, split):
        ...
        for i in range(len(ds)):
            ...
            records.append({
                ...
                "prompt": [{
                    "role": "user",
                    "content": instruction_following(q, a, r)
                }],
    ...
    def is_short_enough(example):
        if "prove" in example.get('query', '').lower(): return False
    ...
    for record in records:
        with open('/home/ec2-user/prismatic-synthesis/prismatic-synthesis/data/datasets/openr1math_seed.jsonl', 'a+') as f:
