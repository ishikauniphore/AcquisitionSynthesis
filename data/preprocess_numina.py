...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--local_save_dir", default="/home/ubuntu/AcquisitionSynthesis/data/numina/", help="The save directory for the preprocessed dataset."
    )
    ...
    full_dataset = full_dataset.filter(lambda x: 'boxed' in x['solution'])
    full_dataset = full_dataset.rename_column("problem", "query")
    full_dataset = full_dataset.rename_column("solution", "reasoning")
    full_dataset = full_dataset.map(lambda x: {'response': extract_last_boxed(x['reasoning'])})
    full_dataset = full_dataset.filter(lambda x: x['response'] is not None)
